...
uint16_t memgethw (uint8_t *memptr, uint32_t addr) {
	if (addr > memmax) {logmsgf(LOGMEM, "MEM: Error attempted mem access (0x%08X) outside of valid memory range (0x%08X)", addr, memmax); return 0;}
...
void memputhw (uint8_t *memptr, uint32_t addr, uint16_t data) {
	if (addr > memmax) {logmsgf(LOGMEM, "MEM: Error attempted mem access (0x%08X) outside of valid memory range (0x%08X)", addr, memmax); return;}
